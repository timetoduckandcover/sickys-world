...
if ( ! class_exists( 'FaceWP_Menu_Widget' ) ) {

	add_action( 'widgets_init', 'load_facewp_menu_widget' );

	function load_facewp_menu_widget() {
		register_widget( 'FaceWP_Menu_Widget' );
	}

	class FaceWP_Menu_Widget extends WP_Widget {

		function __construct() {

			$widget_details = array(
				'classname'   => 'facewp-menu-widget widget_nav_menu',
				'description' => 'Add one of your custom menus as a widget.'
			);

			parent::__construct( 'facewp-menu-widget', __( 'FaceWP Menu', 'facewp' ), $widget_details );
		...
		function form( $instance ) {
			...
			if ( ! $menus ) {
				echo '<p>' . sprintf( __( 'No menus have been created yet. <a href="%s">Create some</a>.', 'facewp' ), admin_url( 'nav-menus.php' ) ) . '</p>';

				return;
			}
			?>
			<p><label
					for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:', 'facewp' ) ?></label><input
					type="text" class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>"
					name="<?php echo $this->get_field_name( 'title' ); ?>" value="<?php echo esc_html( $title ); ?>"/>
			</p>
			<p><label
					for="<?php echo $this->get_field_id( 'nav_menu' ); ?>"><?php _e( 'Select Menu:', 'facewp' ); ?></label>
