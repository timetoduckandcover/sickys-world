...
foreach ( $graph_lines_data as $line ) {
	...
	$output .= '<div class="vc_general vc_single_bar fwp_single_bar' . ( ( isset( $line['color'] ) && 'custom' !== $line['color'] ) ?
			' vc_progress-bar-color-' . $line['color'] : '' )
		. '">';
	$output .= '<h6 class="vc_label"' . $line['txtcolor'] . '>' . $line['label'] . '</h6>';
	if ( $max_value > 100.00 ) {
		$percentage_value = (float) $line['value'] > 0 && $max_value > 100.00 ? round( (float) $line['value'] / $max_value * 100, 4 ) : 0;
	} else {
		$percentage_value = $line['value'];
	}
	$output .= '<div class="vc_bar_container"><div class="vc_bar_inner"><span class="vc_bar ' . esc_attr( implode( ' ', $bar_options ) ) . '" data-percentage-value="' . esc_attr( $percentage_value ) . '" data-value="' . esc_attr( $line['value'] ) . '"' . $line['bgcolor'] . '></span></div></div>';
	$output .= '<h6 class="vc_label"' . $line['txtcolor'] . '>' . $unit . '</h6>';
	$output .= '</div>';
}
